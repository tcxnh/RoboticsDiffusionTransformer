...
def create_model(args, pretrained, dtype=torch.bfloat16, model_type=None, **kwargs):
    model = RoboticDiffusionTransformerModel(args, dtype=dtype, **kwargs)
    if pretrained is not None:
        model.load_pretrained_weights(pretrained, model_type=model_type)
    return model
...
class RoboticDiffusionTransformerModel(object):
    ...
    def load_pretrained_weights(self, pretrained=None, model_type=None):
        ...
        if model_type is None:
            ...
            if filename.endswith('.pt'):
                ...
                raise ValueError(f"Unknown checkpoint format: {pretrained}")
        ...
        if model_type == 'pt':
            ...
            print("Successfully loaded .pt model")
            ...
            try:
                ...
                # 尝试方法1a: 作为pickle加载
                try:
                    print("Attempting to load .bin as pickle file...")
                    state_dict = pickle.loads(bin_data)
                    self.policy.load_state_dict(state_dict)
                    print("Successfully loaded .bin model as pickle")
                    return
                ...
                # 尝试方法1b: 作为torch张量加载
                try:
                    print("Attempting to load .bin as torch file...")
                    buffer = io.BytesIO(bin_data)
                    state_dict = torch.load(buffer, map_location='cpu')
                    if isinstance(state_dict, dict):
                        if "module" in state_dict:
                            self.policy.load_state_dict(state_dict["module"])
                        else:
                            self.policy.load_state_dict(state_dict)
                        print("Successfully loaded .bin model as torch file")
                        return
                    else:
                        print(f"File loaded but not a state dict: {type(state_dict)}")
                except Exception as e:
                    print(f"Failed to load as torch file: {e}")
                
                # 尝试方法2: 使用transformers库加载
                try:
                    print("Attempting to load .bin using transformers...")
                    from transformers import AutoModel
                    temp_model = AutoModel.from_pretrained(os.path.dirname(pretrained))
                    # 将权重映射到当前模型
                    state_dict = temp_model.state_dict()
                    # 可能需要键名映射
                    self.policy.load_state_dict(state_dict, strict=False)
                    print("Successfully loaded .bin model using transformers")
                    return
                except Exception as e:
                    print(f"Failed to load using transformers: {e}")
                
                # 如果还是失败，尝试自定义映射逻辑
                try:
                    print("Attempting to load .bin with custom mapping...")
                    # 假设这是一个HuggingFace格式的.bin文件
                    ...
                    key_mapping = {}
                    # 填充映射关系，例如: 
                    # key_mapping["bert.encoder.layer"] = "transformer.layers"
                    ...
                    print("Successfully loaded .bin model with custom mapping")
                    return
                except Exception as e:
                    print(f"Failed to load with custom mapping: {e}")
                
                raise ValueError(f"All methods failed to load .bin file: {pretrained}")
                
            except Exception as e:
                print(f"Error loading .bin file: {e}")
                raise ValueError(f"Failed to load .bin file: {pretrained}")
        else:
